...
if ($action==ADD)
{
	...
	{
		//1.- Genero la informacion de la visita Guiada
		$sql="INSERT INTO ficha_vg VALUES('','5','".$_POST['pgrn']."','".$_POST['f_inicio']."','".$_POST['f_termino']."',UPPER('".$_POST['lugar']."'),UPPER('".$_POST['objetivo']."'),UPPER('".$_POST['resultado']."'),'".$_POST['aporte_pdss']."','".$_POST['aporte_org']."','".$_POST['aporte_otro']."')";
		...
		//3.- redirecciono
		echo "<script>window.location ='n_vg_mrn.php?SES=$SES&anio=$anio&cod=$codigo&modo=asistente'</script>";
	}
}
elseif($action==ADD_ASISTENTE)
{
	...
	echo "<script>window.location ='n_vg_mrn.php?SES=$SES&anio=$anio&cod=$cod&modo=asistente'</script>";
}
...
echo "<script>window.location ='vg_mrn.php?SES=$SES&anio=$anio&modo=edit'</script>";
}
