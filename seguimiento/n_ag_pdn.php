...
$total=mysql_num_rows($result1);
	...
	if ($total==0)
	{
		...
		if ($total1==0)
		{
			...
			if ($total2==0)
			{
				echo "<script>window.location ='n_ag_pdn.php?SES=$SES&anio=$anio&error=no_dni'</script>";
			}
			else
			{
...
<form name="form5" method="post" action="gestor_ag_pdn.php?SES=<? echo $SES;?>&anio=<? echo $anio;?>&action=ADD" onsubmit="return checkSubmit();">
...
<button type="button" onclick="this.form.action='n_ag_pdn.php?SES=<? echo $SES;?>&anio=<? echo $anio;?>&action=VERIFICAR';this.form.submit()" class="small success button">Verificar DNI</button>
</div>
...
<div class="four columns">
	<select name="calificacion" class="required six">
		<option value="" selected="selected">Seleccionar</option>
		<?
...
<div class="four columns">
	<select name="asignacion" class="required six">
		<option value="" selected="selected">Seleccionar</option>
		<?
