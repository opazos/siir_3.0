...
{
//1.- Actualizo
$sql="UPDATE pit_bd_ficha_pdn SET f_presentacion_2='".$_POST['f_presentacion']."',n_voucher_2='".$_POST['n_voucher']."',monto_organizacion_2='".$_POST['monto_org']."' WHERE cod_pdn='$id'";
$result=mysql_query($sql) or die (mysql_error());

//2.- Redirecciono
echo "<script>window.location ='pdn.php?SES=$SES&anio=$anio&modo=segundo_edit'</script>";
}
...
//3.- Generamos un correo de aviso
	...
	$destinatario = $email;
	$asunto = "MENSAJE DEL SISTEMA - SIIR : BAJA DE INICIATIVA DE PLAN DE NEGOCIO";
	$cuerpo = '
	...
	<head>
	<title>AVISO DE BAJA DE INICIATIVA DE PLAN DE NEGOCIO</title>
	</head>
	<body>
	<h1>MENSAJE DEL SISTEMA - SIIR:</h1>
	<p>
	El usuario '.$r2['nombre'].' '.$r2['apellido'].' a DADO DE BAJA a la INICIATIVA DE PLAN DE NEGOCIO denominada: '.$r3['denominacion'].', perteneciente a la ORGANIZACION '.$r3['nombre'].', de la Oficina Local de '.$r2['oficina'].', en la fecha y hora siguientes: '.date("d-m-Y H:i:s").'
	</p>
...
elseif($action==UPDATE_ESTADO)
{
	...
	foreach($estado as $cad=>$a1)
	{
		...
		$sql="UPDATE pit_bd_ficha_pdn SET cod_estado_iniciativa='".$_POST['estado'][$cad]."' WHERE cod_pdn='$cad'";
		$result=mysql_query($sql) or die (mysql_error());
	}
	//2.- Enviamos un aviso
	$sql="SELECT
	...
	$email_usuario=$r2['correo'];
	$destinatario = $email;
	$asunto = "MENSAJE DEL SISTEMA - SIIR : ACTUALIZACION DE ESTADOS DE PLANES DE NEGOCIO";
	$cuerpo = '
	<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
	<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
	<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
	<head>
	<title>AVISO DE ACTUALIZACION DE ESTADOS DE PLANES DE NEGOCIO</title>
	</head>
	<body>
	<h1>MENSAJE DEL SISTEMA - SIIR:</h1>
	<p>
	El usuario '.$r2['nombre'].' '.$r2['apellido'].' a REALIZADO UNA ACTUALIZACION EN BLOQUE DE LOS ESTADOS SITUACIONALES DE LOS PLANES DE NEGOCIO, de la Oficina Local de '.$r2['oficina'].', en la fecha y hora siguientes: '.date("d-m-Y H:i:s").'
	</p>
