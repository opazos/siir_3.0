...
<form name="form5" class="custom" method="post" action="gestor_rinde_clar.php?SES=<? echo $SES;?>&anio=<? echo $anio;?>&action=ADD" onsubmit="return checkSubmit();">
...
<div class="four columns">
	<select name="cod_clar">
		...
		{
		?>
		<option value="<? echo $r1['cod_clar'];?>"><? echo $r1['cod_clar']."-".periodo($r1['f_evento']);?></option>
		<?
		}
		?>
	</select>
...
<div class="four columns"><input type="date" name="f_rendicion" class="date seven required" placeholder="aaaa-mm-dd" maxlength="10" value="<? echo $fecha_hoy;?>"></div>	

<div class="two columns">Monto devuelto (S/.)</div>
<div class="four columns"><input type="text" name="monto_devuelto" class="number five"></div>
...
<select name="dj">
	...
	{
	?>
	<option value="<? echo $r2['cod_dj_evento'];?>"><? echo numeracion($r2['n_dj_evento'])."-".periodo($r2['f_presentacion']);?></option>
	<?
	}
	?>
</select>
