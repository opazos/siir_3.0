...
<form name="form5" method="post" action="gestor_premia_territorio.php?SES=<? echo $SES;?>&anio=<? echo $anio;?>&cod=<? echo $cod;?>&action=UPDATE" onsubmit="return checkSubmit();">
...
<table>
	...
	<?
		...
		INNER JOIN org_ficha_taz ON org_ficha_taz.cod_tipo_doc = pit_bd_ficha_pit.cod_tipo_doc_taz AND org_ficha_taz.n_documento = pit_bd_ficha_pit.n_documento_taz
	 	...
	 	ORDER BY gcac_pit_participante_concurso.puntaje_total DESC LIMIT 0,$max_ganador";
		$result=mysql_query($sql) or die (mysql_error());
	...
	?>
		<tr>
			...
			<td><? echo number_format($fila['puntaje_total'],2);?></td>
