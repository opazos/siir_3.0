...
<form name="form5" class="custom" method="post" action="gestor_cf.php?SES=<? echo $SES;?>&anio=<? echo $anio;?>&cod=<? echo $cod;?>&tipo=<? echo $tipo;?>&action=ADD_CAL_PUBLIC" onsubmit="return checkSubmit();">
...
<table>
  ...
  <tr>
    <td>Presentación de los resultados obtenidos por el PGRN: manejo de información relevante (CIF, visita guiada, % de familias participantes, otros), paneles fotográficos, muestra de productos,  beneficios que les brinda los temas implementados      </td>
    <td class="centrado">01 a 04 puntos</td>
  ...
  <tr>
    <td>Evidencias presentadas por la organización sobre gestiones desarrolladas que permitan la continuidad de sus iniciativas relacionadas con la gestión de RRNN (Coordinaciones, convenios, apalancamiento de recursos, otros ) </td>
...
if($tipo==1)
{
  $sql="SELECT bd_ficha_publica_cf.total_calif, 
  bd_ficha_publica_cf.cod_ficha, 
  clar_bd_miembro.nombre, 
  clar_bd_miembro.paterno, 
  clar_bd_miembro.materno, 
  ...
  pit_bd_ficha_pit.n_contrato, 
  pit_bd_ficha_pit.f_contrato
FROM bd_ficha_cfinal INNER JOIN bd_ficha_publica_cf ON bd_ficha_cfinal.cod_participante = bd_ficha_publica_cf.cod_participante
   ...
   INNER JOIN org_ficha_taz ON org_ficha_taz.cod_tipo_doc = pit_bd_ficha_pit.cod_tipo_doc_taz AND org_ficha_taz.n_documento = pit_bd_ficha_pit.n_documento_taz
   INNER JOIN bd_jurado_cfinal ON bd_jurado_cfinal.cod_jurado = bd_ficha_publica_cf.cod_jurado
...
elseif($tipo==3)
{
  $sql="SELECT bd_ficha_publica_cf.cod_ficha, 
  bd_ficha_publica_cf.total_calif, 
  clar_bd_miembro.nombre, 
  clar_bd_miembro.paterno, 
  clar_bd_miembro.materno, 
  pit_bd_ficha_pdn.n_contrato, 
...
?>  
    <tr>
      ...
      <td><a href="gestor_cf.php?SES=<? echo $SES;?>&anio=<? echo $anio;?>&cod=<? echo $cod;?>&tipo=<? echo $tipo;?>&id=<? echo $fila['cod_ficha'];?>&action=DELETE_CAL_PUBLIC" class="tiny alert button">Eliminar</a></td>
    </tr>
<?php
}
?>    
