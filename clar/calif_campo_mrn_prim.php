...
<form name="form5" method="post" action="gestor_calif_mrn.php?SES=<? echo $SES;?>&anio=<? echo $anio;?>&id=<? echo $id;?>&action=ADD&modo=PRIMERO_CAMPO" onsubmit="return checkSubmit();">
...
<div class="twelve columns"><hr/></div>
      ...
      <thead>
        ...
        <tr>
          <td rowspan="7">I.- Organización  (CC, Anexo, Sector, Comité de Regantes, Grupo de Interés, otros) </td>
        ...
        <tr>
          ...
          <td rowspan="3" align="center"><input name="p3" type="text" class="required number mini" id="p3" /></td>
        </tr>
        <tr>
          <td>b) De 51% a 74% de las familias que participan en el PGRN (de 2 a 3  puntos)</td>
          </tr>
        <tr>
          <td>c) Menos de 50% de familias que participan en el PGRN (hasta 2 puntos)</td>
          </tr>
        <tr>
          <td>1.4. Mujer y jóvenes</td>
          <td>Percepción del grado de interés y participacion de mujeres y jóvenes en la propuesta del PGRN o como integrantes de la Junta Directiva </td>
          <td align="center"><input name="p4" type="text" class="required number mini" id="p4" /></td>
        </tr>
        <tr>
          <td>1.5.- Planificación y gestión</td>
          <td>Disponibilidad de instrumentos de planificación (planes, proyectos, otros) relacionados con la gestion de rrnn</td>
        ...
        <tr>
          ...
          <td>Percepción sobre la voluntad de destinar el máximo de recursos en el PGRN y posibilidades de incremento de sus activos naturales y financieros a partir de la ejecución del PGRN </td>
...
<table>
	...
	<?
	$num=0;
	$sql="SELECT clar_calif_ficha_mrn.total_puntaje, 
	org_ficha_organizacion.n_documento, 
	org_ficha_organizacion.nombre AS organizacion, 
	clar_bd_miembro.nombre, 
	clar_bd_miembro.paterno, 
	clar_bd_miembro.materno, 
	clar_calif_ficha_mrn.cod_ficha_calif_mrn
FROM clar_bd_ficha_mrn INNER JOIN clar_calif_ficha_mrn ON clar_bd_ficha_mrn.cod_ficha_mrn_clar = clar_calif_ficha_mrn.cod_ficha_mrn_clar
	 INNER JOIN clar_bd_jurado_evento_clar ON clar_bd_jurado_evento_clar.cod_jurado = clar_calif_ficha_mrn.cod_jurado
...
ORDER BY organizacion ASC";
	...
	?>
		<tr>
			...
			<td><a href="gestor_calif_mrn.php?SES=<? echo $SES;?>&anio=<? echo $anio;?>&id=<? echo $id;?>&cod=<? echo $fila['cod_ficha_calif_mrn'];?>&action=DELETE&modo=PRIMERO_CAMPO" class="small alert button">Eliminar</a></td>
		</tr>
	<?
	}
	?>	
