...
<div class="panel">
<div class="row">
<form name="form5" class="custom" method="post" action="gestor_calif_pdn.php?SES=<? echo $SES;?>&anio=<? echo $anio;?>&cod=<? echo $cod;?>&action=ADD" onsubmit="return checkSubmit();">
...
<table>
	...
	<tbody>
		...
		<tr>
			<td>Muestra sus experiencias y conocimientos adquiridos en la ejecucion de su PDN y presenta su archivador</td>
		...
		<tr>
			<td>Se evidencia una diversificacion en la produccion en comparación a la situacion anterior al PDN</td>
		...
		<tr>
			<td>Tienen pensado o están en proceso de convertirse  en una micro empresa</td>
...
<div class="twelve columns">
	...
	<a href="calif_pdn.php?SES=<? echo $SES;?>&anio=<? echo $anio;?>&modo=califica" class="secondary button">Finalizar</a>
</div>
...
<table>
	...
	<?
	$num=0;
	$sql="SELECT gcac_ficha_pdn.cod_ficha, 
	gcac_ficha_pdn.p_total, 
	gcac_participante_concurso.descripcion, 
	org_ficha_organizacion.nombre, 
	gcac_jurado_concurso.nombres, 
	gcac_jurado_concurso.apellidos
FROM gcac_participante_concurso INNER JOIN gcac_ficha_pdn ON gcac_participante_concurso.cod_participante = gcac_ficha_pdn.cod_participante
	 INNER JOIN gcac_jurado_concurso ON gcac_jurado_concurso.cod_jurado = gcac_ficha_pdn.cod_jurado
...
WHERE gcac_participante_concurso.cod_concurso='$cod'";
	...
	?>
		<tr>
			...
			<td><a href="gestor_calif_pdn.php?SES=<? echo $SES;?>&anio=<? echo $anio;?>&cod=<? echo $cod;?>&id=<? echo $fila['cod_ficha'];?>&action=DELETE" class="small alert button">Eliminar</a></td>
		</tr>
	<?
	}
	?>	
