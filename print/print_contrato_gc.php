...
FROM gcac_bd_evento_gc INNER JOIN sys_bd_tipo_iniciativa ON sys_bd_tipo_iniciativa.cod_tipo_iniciativa = gcac_bd_evento_gc.cod_tipo_iniciativa
	 ...
	 LEFT OUTER JOIN sys_bd_fuente_fto ON sys_bd_fuente_fto.cod = gcac_bd_evento_gc.fte_fto
...
//Ahora busco los eventos clar
//a.- busco si tiene en pdn
$sql="SELECT
...
INNER JOIN clar_bd_ficha_pit ON clar_bd_ficha_pit.cod_pit = pit_bd_ficha_pdn.cod_pit
...
INNER JOIN clar_atf_pit ON clar_atf_pit.cod_atf_pit = clar_atf_mrn.cod_atf_pit
...
WHERE gcac_bd_evento_gc.cod_evento_gc='$cod' AND
...
DE DONACIÓN SUJETO A CARGO PARA LA PARTICIPACION EN CONCURSOS DE VALORIZACIÓN DE CONOCIMIENTOS LOCALES Y ACTIVOS CULTURALES EN EL EVENTO "<?php  echo $row['evento'];?>"
</div>
<br>
...
<table width="90%" border="0" align="center" cellpadding="1" cellspacing="1" class="justificado">
  <tr>
    <td colspan="2">Conste por el presente documento el Contrato de Donación sujeto a Cargo para la participación del Conocimiento sobre <?php  echo $row['tipo_evento'];?> que celebran, de una parte EL NÚCLEO EJECUTOR CENTRAL DEL PROYECTO DE  DESARROLLO SIERRA SUR II con RUC Nº 20456188118, en adelante denominado "<? echo $proyecto;?>", representado por el Jefe de la Oficina Local de <?php echo $row['oficina'];?> Sr.(a)<? echo $r2['nombre']." ".$r2['apellido'];?>, identificado(a) con DNI Nº <? echo $r2['n_documento'];?>, con domicilio legal en <? echo $r2['direccion'];?> del Distrito de <? echo $r2['ubicacion'];?>, de la Provincia de <? echo $r2['provincia'];?> y Departamento de <? echo $r2['departamento'];?>; y de otra parte la <?php  echo $row['nombre'];?> con <?php  echo $row['tipo_doc'];?> Nº <?php  echo $row['n_documento'];?>, con domicilio legal en <?php  echo $row['sector'];?> del Distrito de <?php  echo $row['distrito'];?>, Provincia de <?php  echo $row['provincia'];?> y Departamento de <?php  echo $row['departamento'];?>, a quien en adelante se le denominará "<? echo $org;?>", 
    <?php 
    if ($row['cod_tipo_org']==6)
    {
    ?>
...
<?
//1.- si la organizacion es un Plan de negocios
if ($total_pdn<>0)
{
?>
<tr>
...
<td width="97%">
"<? echo $org;?>" ha  participado en el Concurso de Iniciativas – CLAR, realizado el <?php  echo traducefecha($r7['f_evento']);?>, en el Distrito de <?php  echo $r7['distrito'];?>, habiendo logrado el cofinanciamiento del "<?php  echo $proyecto;?>", a través de su iniciativa denominada: <?php  echo $r7['denominacion'];?>.</td>
</tr>
<?
}
//2.- Si la organizacion es un municipio
elseif($row['cod_tipo_org']==6)
{
?>
<tr>
<td valign="top">1.3</td>
<td width="97%">
"<?php  echo $proyecto;?>" y "<?php  echo $org;?>" han suscrito un Convenio de Cooperación Interinstitucional con el objetivo de implementar y cofinanciar actividades conjuntas para el acompañamiento y fortalecimiento de las iniciativas rurales a favor de organizaciones campesinas y otras del ámbito de "<?php  echo $org;?>".
</td>
</tr>
<tr>  
<?
...
<tr>
<td valign="top">1.3</td>
<td width="97%"><? echo $row['detalle'];?>.</td>
</tr>
<?
}
?>  
  ...
  <tr>
    ...
    <td width="97%">"<?php  echo $org;?>", ha presentado a la Oficina Local de <?php  echo $row['oficina'];?>, una propuesta para participar en el Concurso denominado:  <?php  echo $row['evento'];?>, que se llevará a cabo el día <?php  echo traducefecha($row['f_evento']);?> en <?php  echo $row['lugar'];?> y cuenta con la opinión favorable emitida por el Jefe de la Oficina Local de <?php echo $row['oficina'];?>. La propuesta aprobada debidamente firmada por las partes forma parte del presente Contrato </td>
  </tr>
  <tr class="txt_titulo">
    <td colspan="2">CLAUSULA SEGUNDA: OBJETO DEL CONTRATO</td>
  </tr>
  <tr>
    <td colspan="2">Por el presente contrato "<? echo $proyecto;?>" transfiere en donación sujeto a cargo, el monto total de S/. <?php  echo number_format($row['aporte_pdss'],2)?> (<?php  echo vuelveletra($row['aporte_pdss'])?> Nuevos Soles) a "<? echo $org;?>", la misma que se compromete a aportar el monto total de S/. <?php  echo number_format($row['aporte_org'],2);?> (<?php  echo vuelveletra($row['aporte_org'])?> Nuevos Soles). Ambos montos serán destinados para financiar la participación en el Concurso referido en el ítem 1.4 dela Claúsula Primera, según el siguiente cuadro:</td>
  </tr>
</table>
<br>
<table width="90%" border="1" align="center" cellpadding="1" cellspacing="1" class="mini">
  <tr class="centrado txt_titulo">
    ...
    <td width="10%">APORTE SIERRA SUR II (S/.)</td>
    <td width="10%">APORTE ORGANIZACIÓN (S/.)</td>
    <td width="10%">APORTE OTROS (S/.)</td>
  ...
  <tr>
    ...
    <td class="derecha"><? $pp_otr=$row['aporte_otro']/$total_evento*100; echo number_format($pp_otr,2);?></td>
    <td class="derecha"><? echo number_format($pp_pdss+$pp_org+$pp_otr,2);?></td>
  </tr>
</table>
<br>
<table width="90%" border="0" align="center" cellpadding="1" cellspacing="1" class="justificado">
  ...
  <tr>
    <td colspan="2">El plazo establecido por las partes para la ejecución del presente contrato es de 3 meses; se inicia el <?php  echo traducefecha($row['f_contrato']);?> y culmina el <?php  echo traducefecha($fecha_termino);?> Este plazo incluye las acciones de liquidación del Contrato y perfeccionamiento de la donación.</td>
  </tr>
  <tr class="txt_titulo">
...
A "<? echo $org;?>" PARA PARTICIPACIÓN EN CONCURSO DE VALORIZACIÓN DE CONOCIMIENTOS LOCALES Y ACTIVOS CULTURALES</div>
...
<table width="90%" border="0" align="center" cellpadding="1" cellspacing="1">
  ...
  <tr>
    ...
    <td width="1%">:</td>
    <td width="76%">Desembolso  para participación en concursos de valorización de conocimientos locales y activos culturales</td>
  </tr>
  <tr>
    <td><? echo $org;?></td>
    <td width="1%">:</td>
...
<table width="90%" border="0" align="center" cellpadding="4" cellspacing="4">
  <tr>
    ...
    <a href="../contratos/contrato_gc.php?SES=<? echo $SES;?>&anio=<? echo $anio;?>&modo=imprime" class="secondary button oculto">Finalizar</a>
