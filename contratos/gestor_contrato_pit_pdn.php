...
if ($_POST['pit']==NULL)
{
echo "<script>window.location ='n_contrato_pit_pdn.php?SES=$SES&anio=$anio&modo=pit&error=vacio'</script>";
}
else
{
...
$sql="UPDATE sys_bd_numera_dependencia SET n_contrato_iniciativa='".$_POST['n_contrato']."',n_solicitud_iniciativa='".$_POST['n_solicitud']."' WHERE cod='".$_POST['cod_numeracion']."'";
$result=mysql_query($sql) or die (mysql_error());
...
//4.- Generamos la ATF
 $sql="INSERT INTO clar_atf_pit VALUE('','0','3','$poa','0','0','','0','".$_POST['pit']."')";
 $result=  mysql_query($sql) or die (mysql_error());

$codigo=$_POST['pit'];

//5.- Redireccionamos
	echo "<script>window.location ='n_contrato_pit_pdn.php?SES=$SES&anio=$anio&cod=$codigo&modo=pdn'</script>";
}
}
...
if ($_POST['pdn']==NULL)
{
echo "<script>window.location ='n_contrato_pit_pdn.php?SES=$SES&anio=$anio&cod=$cod&modo=pdn&error=vacio'</script>";
}
else
{
//1.- Ubico la ATF del PIT
$sql="SELECT clar_atf_pit.cod_atf_pit
FROM clar_atf_pit
WHERE clar_atf_pit.cod_pit='$cod'";
$result=mysql_query($sql) or die (mysql_error());
$r3=mysql_fetch_array($result);


//2.- Actualizo la numeracion de la oficina
...
$sql="INSERT INTO clar_atf_pdn VALUES('','1','','','','".$_POST['n_atf']."','2','$poa_1','$poa_2','$poa_3','$poa_4','".$r1['monto_1']."','".$r1['saldo_1']."','".$r1['monto_2']."','".$r1['saldo_2']."','".$r1['monto_3']."','".$r1['saldo_3']."','".$r1['monto_4']."','".$r1['saldo_4']."','".$_POST['pdn']."','".$r3['cod_atf_pit']."')";
$result=mysql_query($sql) or die (mysql_error());
...
//4.- Redirecciono
	echo "<script>window.location ='n_contrato_pit_pdn.php?SES=$SES&anio=$anio&cod=$cod&modo=pdn'</script>";
}
}
elseif($action==UPDATE_ATF_PDN)
{
//1.- Ubico la info del ATF del Plan de Negocio
$sql="SELECT * FROM clar_atf_pdn WHERE cod_atf_pdn='$id'";
$result=mysql_query($sql) or die (mysql_error());
$r2=mysql_fetch_array($result);


//2.- Realizo los calculos del Plan de negocio
$sql="SELECT (pit_bd_ficha_pdn.at_pdss*0.70) as monto_1, 
	(pit_bd_ficha_pdn.vg_pdss*0.70) as monto_2, 
	(pit_bd_ficha_pdn.fer_pdss*0.70) as monto_3, 
	(pit_bd_ficha_pdn.total_apoyo*0.70) as monto_4,
	...
	(pit_bd_ficha_pdn.total_apoyo*0.30) as saldo_4
FROM pit_bd_ficha_pdn
WHERE pit_bd_ficha_pdn.cod_pdn='".$r2['cod_pdn']."'";
$result=mysql_query($sql) or die (mysql_error());
$r1=mysql_fetch_array($result);
...
//4.- Actualizo el ATF del Plan de Negocio
...
//5.- Redirecciono la informacion
echo "<script>window.location ='n_contrato_pit_pdn.php?SES=$SES&anio=$anio&cod=$cod&modo=pdn&alert=success_change'</script>";
...
//1.- Guardo la informacion del PIT
$sql="UPDATE pit_bd_ficha_pit SET n_contrato='".$_POST['n_contrato']."',f_contrato='".$_POST['f_contrato']."',mes='".$_POST['duracion']."',f_termino='$f_termino',n_solicitud='".$_POST['n_solicitud']."',fuente_fida='".$_POST['fte_fida']."',fuente_ro='".$_POST['fte_ro']."' WHERE cod_pit='".$_POST['codigo']."'";
$result=mysql_query($sql) or die (mysql_error());
...
elseif($action==ANULA)
{
	...
	//2.- Actualizamos el estado de los Planes de Negocio que conforman el PIT
	$sql="UPDATE pit_bd_ficha_pdn SET cod_estado_iniciativa='000' WHERE cod_pit='$id'";
	$result=mysql_query($sql) or die (mysql_error());
	
	//3.- Busco los datos del PIT
	$sql="SELECT
	...
	//5.- Redirecciono
	echo "<script>window.location ='contrato_pit_pdn.php?SES=$SES&anio=$anio&modo=anula&alert=success_change'</script>";
}
