...
$sql="SELECT * FROM ml_pf WHERE cod_evento='$id'";
$result=mysql_query($sql) or die (mysql_error());
...
<form name="form5" method="post" action="gestor_contrato_pf.php?SES=<? echo $SES;?>&anio=<? echo $anio;?>&action=UPDATE" onsubmit="return checkSubmit();">
...
{
?>
<option value="<? echo $fila['cod_tipo_doc'].",".$fila['n_documento'];?>" <? if ($fila['cod_tipo_doc']==$row1['cod_tipo_doc_org'] and $fila['n_documento']==$row1['n_documento_org']) echo "selected";?>><? echo $fila['nombre'];?></option>
<?
}
?>
</select>
...
<div class="ten columns"><input type="text" name="evento" class="ten required" value="<? echo $row1['nombre'];?>"></div>
<div class="two columns">Fecha del evento</div>
<div class="four columns"><input type="date" name="f_inicio" class="seven required date" placeholder="aaaa-mm-dd" maxlength="10" value="<? echo $row1['f_evento'];?>"></div>
<div class="two columns">Nº de días de duración</div>
<div class="four columns"><input type="text" name="dia" class="required digits five" value="<? echo $row1['dia'];?>"></div>


<div class="two columns">Participantes</div>
<div class="four columns"><input type="text" name="participantes" class="five required digits" value="<? echo $row1['n_participante'];?>"></div>
<div class="two columns">Lugar</div>
<div class="four columns"><input type="text" name="lugar" class="ten required" value="<? echo $row1['lugar'];?>"></div>
...
<div class="twelve columns">
	<textarea id="elm2" name="resultado" rows="5"  style="width: 100%">
		<? echo $row1['resultados'];?>
	</textarea>
</div>

<div class="twelve columns"><h6>V.- Datos del contrato a firmar</h6></div>
<div class="two columns">Nº de contrato</div>
<div class="four columns"><input type="text" name="n_contrato" class="required digits five" value="<? echo $row1['n_contrato'];?>"></div>
<div class="two columns">Fecha de contrato</div>
<div class="four columns"><input type="date" name="f_contrato" class="required date seven" placeholder="aaaa-mm-dd" maxlength="10" value="<? echo $row1['f_contrato'];?>"></div>
<div class="two columns">Nº de Solicitud</div>
<div class="four columns"><input type="text" name="n_solicitud" class="required digits five" value="<? echo $row1['n_solicitud'];?>"></div>
<div class="two columns">Nº de ATF</div>
<div class="four columns"><input type="text" name="n_atf" class="required digits five" value="<? echo $row1['n_atf'];?>"></div>
<div class="two columns">Nº de cuenta bancaria</div>
<div class="four columns"><input type="text" name="n_cuenta" class="required seven" value="<? echo $row1['n_cuenta'];?>"></div>
...
<div class="four columns">
	<select name="ifi">
		...
		{
		?>
		<option value="<? echo $r1['cod_ifi'];?>" <? if ($r1['cod_ifi']==$row1['cod_ifi']) echo "selected";?>><? echo $r1['descripcion'];?></option>
		<?
		}
		?>
	</select>
...
<div class="ten columns">
	<select name="poa" class="hyjack">
		...
		{
		?>
		<option value="<? echo $r2['cod'];?>" <? if ($r2['cod']==$row1['cod_subactividad']) echo "selected";?>><? echo $r2['codigo']."-".$r2['nombre'];?></option>
		<?
		}
		?>
	</select>
...
<div class="two columns"><input type="text" name="aporte_pdss" class="required number ten" value="<? echo $row1['aporte_pdss'];?>"></div>
<div class="two columns">Aporte Organizacion (S/.)</div>
<div class="two columns"><input type="text" name="aporte_org" class="required number ten" value="<? echo $row1['aporte_org'];?>"></div>
<div class="two columns">Aporte Otro (S/.)</div>
<div class="two columns"><input type="text" name="aporte_otro" class="required number ten" value="<? echo $row1['aporte_otro'];?>">
	<input type="hidden" name="codigo" value="<? echo $row1['cod_evento'];?>">
</div>
