...
$sql="SELECT * FROM ml_promocion_c WHERE cod_evento='$id'";
$result=mysql_query($sql) or die (mysql_error());
...
<!-- Contenedores -->
<dl class="tabs">
<dd  class="active"><a href="">Registro de contrato</a></dd>
</dl>
<!-- Termino contenedores -->
<ul class="tabs-content">
<li class="active" id="simple1Tab">
<div class="row collapse">
<form name="form5" method="post" action="gestor_contrato_pc.php?SES=<? echo $SES;?>&anio=<? echo $anio;?>&action=UPDATE" onsubmit="return checkSubmit();">
...
<input type="hidden" name="cod_numera" value="<? echo $r1['cod'];?>">
	<select name="org" class="hyjack">
		...
		{
		?>
		<option value="<? echo $f1['cod_tipo_doc'].",".$f1['n_documento'];?>" <? if ($f1['cod_tipo_doc']==$row['cod_tipo_doc_org'] and $f1['n_documento']==$row['n_documento_org']) echo "selected";?>><? echo $f1['n_documento']." - ".$f1['nombre'];?></option>
		<?
		}
		?>
	</select>
...
<div class="four columns"><input type="text" name="evento" class="required ten" value="<? echo $row['nombre'];?>">
<input type="hidden" name="codigo" value="<? echo $row['cod_evento'];?>">
</div>
...
echo "<select  name='select1' id='select1' onChange='cargaContenido(this.id)' class='five'>";
...
?>
	...
	<div class="four columns">
<select name="select2" id="select2" class="five">
...
?>
    ...
    <div class="four columns">
<select  name="select3" id="select3" class="five">
...
?>
    ...
    <div class="four columns"><input type="text" name="direccion" value="<? echo $row['lugar'];?>"></div>
...
<div class="four columns"><input type="text" name="n_contrato" class="required digits five" value="<? echo $row['n_contrato'];?>"></div>
<div class="two columns">Fecha de contrato</div>
<div class="four columns"><input type="date" name="f_contrato" class="required date five" value="<? echo $row['f_contrato'];?>"></div>
...
<div class="four columns"><input type="text" name="n_cuenta" class="required five" value="<? echo $row['n_cuenta'];?>"></div>
...
<div class="four columns"><input type="text" name="fida" class="required number seven" value="<? echo $row['fte_fida'];?>"></div>
<div class="two columns">Fuente RO %</div>
<div class="four columns"><input type="text" name="ro" class="required number seven" value="<? echo $row['fte_ro'];?>"></div>
...
<div class="four columns"><input type="text" name="aporte_pdss" class="six required number" value="<? echo $row['aporte_pdss'];?>"></div>
<div class="four columns"><input type="text" name="aporte_org" class="six required number" value="<? echo $row['aporte_org'];?>"></div>
<div class="four columns"><input type="text" name="aporte_otro" class="six required number" value="<? echo $row['aporte_otro'];?>"></div>
