...
$sql="SELECT * FROM clar_bd_ficha_sd_pit WHERE cod_ficha_sd='$id'";
$result=mysql_query($sql) or die (mysql_error());
...
{
?>
<form name="form5" class="custom" method="post" action="gestor_pit_sd.php?SES=<? echo  $SES;?>&anio=<? echo $anio;?>&action=UPDATE" onsubmit="return checkSubmit();">	
	...
	<div class="four columns">
		<select name="clar" class="medium">
			...
			{
			?>
			<option value="<? echo $f2['cod_clar'];?>" <? if ($f2['cod_clar']==$row['cod_clar']) echo "selected";?>><? echo numeracion($f2['cod_clar'])."-".periodo($f2['f_evento'])." / ".$f2['oficina'];?></option>
			<?
			}
			?>
		</select>
	...
	<div class="four columns">
	<input type="text" name="n_solicitud" class="required digits seven" value="<? echo $row['n_solicitud'];?>">
	<input type="hidden" name="codigo" value="<? echo $row['cod_ficha_sd'];?>">

	</div>
	<div class="two columns">Fecha de desembolso</div>
	<div class="four columns"><input type="date" name="f_desembolso" class="date required seven" placeholder="aaaa-mm-dd" maxlength="10" value="<? echo $row['f_desembolso'];?>"></div>
	...
	<div class="twelve columns">
		...
		<a href="pit_sd.php?SES=<? echo $SES;?>&anio=<? echo $anio;?>&modo=edit" class="primary button">Cancelar</a>
	</div>	
</form>
<?
}
...
<div class="twelve columns">
	<button type="submit" class="success button" id="btn_envia">Guardar cambios</button>
	<a href="m_pit_sd.php?SES=<? echo $SES;?>&anio=<? echo $anio;?>&cod=<? echo $cod;?>&modo=pdn" class="primary button">Siguiente >></a>
