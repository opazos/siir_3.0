...
$sql="SELECT * FROM pit_bd_ficha_adenda WHERE cod_ficha_adenda='$id'";
$result=mysql_query($sql) or die (mysql_error());
...
{
?>
<form name="form5" class="custom" method="post" action="gestor_adenda_pit.php?SES=<? echo $SES;?>&anio=<? echo $anio;?>&id=<? echo $id;?>&action=UPDATE" onsubmit="return checkSubmit();">

<div class="two columns">Nº de adenda</div>
<div class="ten columns"><input type="text" name="n_adenda" class="two required digits" readonly="readonly" value="<? echo $row2['n_adenda'];?>">
...
<div class="four columns">
	<select name="pit">
		...
		{
		?>
		<option value="<? echo $f1['cod_pit'];?>" <? if ($f1['cod_pit']==$row2['cod_pit']) echo "selected";?>><? echo numeracion($f1['n_contrato'])."-".periodo($f1['f_contrato']);?></option>
		<?
			}
		?>
	</select>
...
<div class="four columns"><input type="date" name="f_adenda" placeholder="aaaa-mm-dd" maxlength="10" class="required date six" value="<? echo $row2['f_adenda'];?>"></div>
<div class="two columns">Tipo de iniciativa a la que se le hara adenda</div>
<div class="four columns">
	<select name="tipo_iniciativa">
		<option value="" selected="selected">Seleccionar</option>
		<option value="3" <? if ($row2['cod_tipo_iniciativa']==3) echo "selected";?>>PIT</option>
		<option value="4" <? if ($row2['cod_tipo_iniciativa']==4) echo "selected";?>>PDN</option>
		<option value="5" <? if ($row2['cod_tipo_iniciativa']==5) echo "selected";?>>PGRN</option>
	</select>
</div>
<div class="two columns">Documento de Referencia</div>
<div class="four columns"><input type="text" name="referencia" class="required nine" value="<? echo $row2['referencia'];?>"></div>
...
<div class="six columns">
	<select name="adenda_plazo">
		<option value="" selected="selected">Seleccionar</option>
		<option value="1" <? if ($row2['tipo_plazo']==1) echo "selected";?>>En esta addenda se ampliará el plazo de duración de la iniciativa</option>
		<option value="2" <? if ($row2['tipo_plazo']==2) echo "selected";?>>En esta addenda se reducirá el plazo de duración de la iniciativa</option>
		<option value="0" <? if ($row2['tipo_plazo']==0) echo "selected";?>>No se modificará la duración de la iniciativa</option>
	</select>
</div>
<div class="twelve columns"></div>

<div class="two columns">Nº de meses a ampliar o reducir</div>
<div class="four columns"><input type="text" name="n_meses" class="digits five" value="<? echo $row2['n_meses'];?>"></div>
<div class="two columns">Nueva fecha de termino del contrato</div>
<div class="four columns"><input type="date" name="f_termino" class="date six" placeholder="aaaa-mm-dd" maxlength="10" value="<? echo $row2['f_termino'];?>"></div>
<div class="twelve columns"><br/></div>
...
<div class="six columns">
	<select name="adenda_monto">
		<option value="" selected="selected">Seleccionar</option>
		<option value="1" <? if ($row2['tipo_monto']==1) echo "selected";?>>En esta addenda se ampliará el monto solicitado por la iniciativa</option>
		<option value="2" <? if ($row2['tipo_monto']==2) echo "selected";?>>En esta addenda se reducirá el monto solicitado por la iniciativa</option>
		<option value="0" <? if ($row2['tipo_monto']==0) echo "selected";?>>No se modificarán los montos solicitados por la iniciativa</option>
	</select>
</div>
<div class="twelve columns"></div>
...
<div class="twelve columns">
	<textarea name="comentario" rows="5"><? echo $row2['comentario'];?></textarea>
</div>
<div class="twelve columns"><br/></div>
<div class="twelve columns">
