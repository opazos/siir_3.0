...
$sql="SELECT pit_bd_ficha_adenda.n_solicitud, 
...
WHERE pit_bd_ficha_adenda.cod_adenda='$id'";
$result=mysql_query($sql) or die (mysql_error());
...
<form name="form5" method="post" action="gestor_adenda_monto.php?SES=<? echo $SES;?>&anio=<? echo $anio;?>&id=<? echo $id;?>&action=UPDATE_PAGO" onsubmit="return checkSubmit();">

<div class="twelve columns"><h6>I.- Datos de la solicitud de desembolso</h6></div>	
<div class="two columns">Nº de Solicitud</div>
<div class="four columns"><input type="text" name="n_solicitud" class="required digits five" value="<? echo $r1['n_solicitud'];?>" readonly="readonly"><input type="hidden" name="codigo" value="<? echo $r1['cod_adenda'];?>"></div>	
<div class="two columns">Fecha de Solicitud</div>
<div class="four columns"><input type="date" name="f_solicitud" class="required date seven" placeholder="aaaa-mm-dd" maxlength="10" value="<? echo $r1['f_solicitud'];?>"></div>
<div class="two columns">Fte FIDA %</div>
<div class="four columns"><input type="text" name="fte_fida" class="required five number" value="<? echo $r1['fte_fida'];?>"></div>
<div class="two columns">Fte RO %</div>
<div class="four columns"><input type="text" name="fte_ro" class="required five number" value="<? echo $r1['fte_ro'];?>"></div>
...
WHERE pit_adenda_pit.cod_adenda='$id'";
	...
	?>
		<tr>
			<td><input type="text" name="n_atf_pit[<? echo $cad;?>]" class="required digits nine" value="<? echo $f1['n_atf'];?>" readonly></td>
...
WHERE pit_adenda_mrn.cod_adenda='$id'";
	...
	?>
		<tr>
			<td><input type="text" name="n_atf_mrn[<? echo $f2['cod_iniciativa'];?>]" class="required digits nine" value="<? echo $f2['n_atf'];?>" readonly></td>
			<td><? echo $f2['nombre'];?></td>
			<td><input type="text" name="n_voucher[<? echo $f2['cod_iniciativa'];?>]" class="required nine" value="<? echo $f2['n_voucher'];?>"></td>
			<td><input type="text" name="monto_aporte[<? echo $f2['cod_iniciativa'];?>]" class="required number nine" value="<? echo $f2['deposito_org'];?>"></td>
			<td><a href="" class="small success button">Actualizar</a></td>
		</tr>
	<?
	}
	...
	<div class="twelve columns">
		...
		<a href="adenda_monto.php?SES=<? echo $SES;?>&anio=<? echo $anio;?>&modo=edita_pago" class="secondary button">Finalizar</a>	
	</div>
