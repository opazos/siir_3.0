...
$sql="SELECT *
FROM ml_liquida_pc
WHERE ml_liquida_pc.cod_liquida_pc='$id'";
$result=mysql_query($sql) or die (mysql_error());
...
<form name="form5" class="custom" method="post" action="gestor_contrato_pc.php?SES=<? echo $SES;?>&anio=<? echo $anio;?>&id=<? echo $id;?>&action=UPDATE_LIQUIDA" onsubmit="return checkSubmit();">
...
<div class="four columns">
	<select name="n_contrato">
		...
		{
		?>
		<option value="<? echo $f1['cod_evento'];?>" <? if ($f1['cod_evento']==$row['cod_evento']) echo "selected";?>><? echo numeracion($f1['n_contrato'])."-".periodo($f1['f_contrato']);?></option>
		<?
		}
		?>
	</select>
...
<div class="four columns"><input type="date" name="f_liquidacion" class="date required five" placeholder="aaaa-mm-dd" maxlength="10" value="<? echo $row['f_rendicion'];?>"></div>
...
<div class="four columns"><input type="text" name="publico" class="required digits five" value="<? echo $row['publico'];?>"></div>
<div class="two columns">Ingresos netos generados</div>
<div class="ten columns"><input type="text" name="ingreso" class="required number two" value="<? echo $row['ingresos'];?>"></div>
<div class="two columns">Monto ejecutado NEC PDSS II</div>
<div class="four columns"><input type="text" name="ejec_pdss" class="required number five" value="<? echo $row['ejec_pdss'];?>"></div>
<div class="two columns">Monto ejecutado Organizacion</div>
<div class="four columns"><input type="text" name="ejec_org" class="required number five" value="<? echo $row['ejec_org'];?>"></div>
</div>
...
WHERE ml_presupuesto_pc.cod_liquida_pc='$id'";
$result=mysql_query($sql) or die (mysql_error());
while($r1=mysql_fetch_array($result))
{
	$cad=$r1['cod_presupuesto'];
?>		
		<tr>
			<td><input type="text" name="conceptos[<? echo $cad;?>]" value="<? echo $r1['descripcion'];?>"></td>
			<td><input type="text" name="unidads[<? echo $cad;?>]" value="<? echo $r1['unidad'];?>"></td>
			<td><input type="text" name="costos[<? echo $cad;?>]" value="<? echo $r1['costo_unitario'];?>"></td>
			<td><input type="text" name="cantidads[<? echo $cad;?>]" value="<? echo $r1['cantidad'];?>"></td>
		</tr>
<?
}
?>		
...
<div class="twelve columns"><textarea name="problema" rows="5"><? echo $row['problema'];?></textarea></div>
<div class="twelve columns"><h6>Apoyo de otras entidades</h6></div>
<div class="two columns">Nombre de la entidad</div>
<div class="four columns"><input type="text" name="entidad" class="ten" value="<? echo $row['entidad_otro'];?>"></div>
<div class="two columns">Monto con el que apoyó (S/.)</div>
<div class="four columns"><input type="text" name="apoyo" class="number five" value="<? echo $row['ejec_otro'];?>"></div>
