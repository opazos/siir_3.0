...
$aporte_total=$r3['aporte_pdss']+$r3['aporte_org']+$r4['aporte_pdss']+$r4['aporte_org']+$r5['aporte_pdss']+$r5['aporte_org'];
...
<form name="form5" method="post" action="gestor_adenda_plazo.php?SES=<? echo $SES;?>&anio=<? echo $anio;?>&cod=<? echo $cod;?>&action=EDIT" onsubmit="return checkSubmit();">
...
<p><strong>CUARTO:</strong>   	Con fecha <? echo traducefecha($row['f_adenda']);?>, LA ORGANIZACIÓN ha solicitado a SIERRA SUR II una ampliación en el plazo de vigencia del PIT en concordancia a lo precisado en el numeral TERCERO de la presente.</p>
...
<p><strong>SEXTO: </strong>  	El objeto de la presente Addenda al Contrato   es modificar la CLAUSULA TERCERA del Contrato  Nº <? echo numeracion($row['n_contrato']);?> – <? echo periodo($row['f_contrato']);?> - PIT – OL <? echo $row['oficina'];?>, en los términos siguientes:</p>
...
<div class="twelve columns">
	...
	<a href="adenda_plazo.php?SES=<? echo $SES;?>&anio=<? echo $anio;?>&modo=imprime" class="primary button">Finalizar</a>
